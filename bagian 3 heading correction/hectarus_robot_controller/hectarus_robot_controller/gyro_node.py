...
class MyNode(Node):
    def __init__(self):
        ...
        self.subscriber_turn_on = self.create_subscription(Int32, "/turn_on_", self.turn_on_roll, 1)
    ...
    def getting_data(self, message:Int32MultiArray):
        ...
        self.send_yaw = self.yaww + (90*self.calibrate) + self.correction
        info.data = [int(self.send_yaw), int(self.roll), int(message.data[0]), int(message.data[1])]
        #self.get_logger().info("Sending Gyro Yaw = " + str(info.data[0]) + ", Sending Gyro Roll = " + str(info.data[1]) + ", and State = " + str(info.data[2]))
    ...
    def ngitung_gyro(self):
        ...
        GyroX = GyroX - self.IMUX #0.023018439999999994 # cek imu error
        GyroY = GyroY - self.IMUY #0.11997028000000012 # cek imu error
        GyroZ = GyroZ - self.IMUZ #(-0.09009203999999987) # cek imu error
        AccelX = data[6] - self.IMUAX
        AccelY = data[7] - self.IMUAY
        AccelZ = 9.80665 + data[8] - self.IMUAZ
        AccelAngleX = ((math.atan((AccelY)/math.sqrt(math.pow((AccelX),2) + math.pow((AccelZ),2))) * 180/math.pi)) #- self.IMUAX #0.6516672554935431 #cek imu error
        AccelAngleY = ((math.atan(-1 * (AccelX)/math.sqrt(math.pow((AccelY),2) + math.pow((AccelZ),2)))*180/math.pi)) #- self.IMUAY #4.8897750962300135 # cek imu error
...
#        self.gyroAngleY = self.gyroAngleY + GyroY * self.elapsedTime
        ...
        self.yaww = (self.yaw * 7.98580301686)
        if self.turn_on == 1:
            self.gyroAngleX = self.gyroAngleX + GyroX * self.elapsedTime
            self.roll = (0.98 * self.gyroAngleX + 0.02 * AccelAngleX) * 7.258065
