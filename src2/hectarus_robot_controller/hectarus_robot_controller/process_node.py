...
class MyNode(Node):
    ...
    def process(self):
        self.ultrasonik[0] = ultrasonic(TRIG[0], ECHO[0])
        if self.flag == True:
            self.constraint = 3
        else:
            self.constraint = 3
        self.get_logger().info("Jarak Ultrasonik Depan: " + str(self.ultrasonik[0]))
        self.get_logger().info("Constraint " + str(self.constraint))
        state = Int32()
        calibrate = Int32()
        if self.ultrasonik[0] <= self.constraint:
            wait(2)
            state.data = 9 #berhenti dulu
            self.publish_state.publish(state) #kirim berhenti
            wait(3)
            self.ultrasonik[1] = ultrasonic(TRIG[2], ECHO[2])
            self.ultrasonik[2] = ultrasonic(TRIG[5], ECHO[5])
            self.get_logger().info("Jarak Ultrasonik Kanan : " + str(self.ultrasonik[1]))
            self.get_logger().info("Jarak Ultrasonik Kiri  : " + str(self.ultrasonik[2]))
            wait(1)
            #self.get_logger().info("Ada halangan didepan")
            if self.ultrasonik[1] < self.ultrasonik[2]:
                ...
                self.publish_state.publish(state)
                wait(3)
            elif self.ultrasonik[2] < self.ultrasonik[1]:
