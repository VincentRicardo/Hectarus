...
class MyNode(Node):
    def __init__(self):
        ...
        self.publish_turn_on_roll = self.create_publisher(Int32, "/turn_on_", 1)
        ...
        self.stop = 2
        self.add_delay = 0
        self.constraint_left = [11,9] #[25,16] #[11,9]
    ...
    def process(self):
        ...
        if self.flag == True and self.trigger_strafe != -4: #flag buat ganti detect depan (kondisi setelah naik tangga)
            self.constraint = 30 #???
            self.trigger_strafe = -4
            self.constraint_left = [25,16]
            self.add_delay = 0.4
            wait(5)
...
#            return
        ...
        if self.jarak[0] <= self.constraint: #kalo jarak depan lebih kecil dari constraintnya
            ...
            wait(1)
            if self.trigger_strafe == -2 or self.trigger_strafe == -4: #jalan strafe ke kiri
                ...
                while True:
                    ...
                    if self.jarak[2] <= self.constraint_left[0] and self.jarak[2] >= self.constraint_left[1]:
                        if self.trigger_strafe == -2: #setelah strafe kiri, yawnya berubah, constraintnya berubah
                            self.trigger_strafe = 3
                            self.constraint = 20
                            correction = Int32()
                            correction.data = 20
...
#                            self.stop = 1
                            #end of line#
                            break
                        elif self.trigger_strafe == -4: #udah mentok kiri sampai titik finish
                            #looping stop
                            #berhenti trus ngeluarin time elapsed#
                            count_time = Int32()
                            count_time.data = 1
                            ...
                            #end of line#
                            break
            elif self.trigger_strafe == 3: #setelah selesai bagian 3, yawnya berubah balik, constraintnya diperpendek, rollnya diaktifin
                self.constraint = 0
                flag_data = Int32()
                flag_data.data = 0
