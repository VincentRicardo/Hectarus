...
#                self.get_logger().info(f"Step_wave: {step_wave}")
        ...
        if message.data[2] == 11:
            ...
            if self.flag_stop_tangga == 2:
                delay = 3
                self.roll = -6
#                self.roll_kaki_kiri = -6
#                if message.data[4] >= 0 and (self.temp >= 3 or self.temp <= -3):
#                    if self.temp >= 0:
#                        self.roll_kaki_kiri = 18
#                    else:
#                        self.roll_kaki_kiri = -22
            else:
                delay = 1
                self.roll = 0
#                self.roll_kaki_kiri = 0
...
#            base_coxa_depan_berdiri_strafe, coxa_femur_depan_berdiri_strafe, femur_tibia_depan_berdiri_strafe = IK_depan(0,0, panjang_depan_belakang/2, max(-6,self.roll_kaki_kiri), 0)
#            base_coxa_belakang_berdiri_strafe, coxa_femur_belakang_berdiri_strafe, femur_tibia_belakang_berdiri_strafe = IK_belakang(0,0, panjang_depan_belakang/2, min(-6,self.roll_kaki_kiri), self.mundur, 0)
