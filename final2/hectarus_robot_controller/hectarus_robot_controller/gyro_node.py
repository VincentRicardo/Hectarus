...
class MyNode(Node):
    ...
    def getting_data(self, message:Int32MultiArray):
        ...
        info.data = [int(self.send_yaw), int(self.roll), int(message.data[0]), int(message.data[1]), int(self.pitch)]
    ...
    def ngitung_gyro(self):
        ...
        if self.turn_on_pitch == 1:
            ...
            self.pitch = ((0.98 * self.gyroAngleY + 0.02 * AccelAngleY)*7.377049)
