...
class MyNode(Node):                      
    ...
    def ngitung_gyro(self):
        ...
        GyroX = GyroX - 0.023018439999999994 # cek imu error
        GyroY = GyroY - 0.11997028000000012 # cek imu error
        GyroZ = GyroZ - (-0.09009203999999987) # cek imu error
        AccelX = data[6]
        AccelY = data[7]
        AccelZ = data[8]
        AccelAngleX = ((math.atan((AccelY)/math.sqrt(math.pow((AccelX),2) + math.pow((AccelZ),2))) * 180/math.pi)) - 0.6516672554935431 #cek imu error
        AccelAngleY = ((math.atan(-1 * (AccelX)/math.sqrt(math.pow((AccelY),2) + math.pow((AccelZ),2)))*180/math.pi)) - 4.8897750962300135 # cek imu error
